...
const EventCard = ({ title, image, slug, location, date, time, description, mode, audience }: Props) => {
      ...
      <div className="relative h-full w-full transition-all duration-700 transform-style-3d group-hover:rotate-y-180 rounded-2xl shadow-xl">
        ...
        <div className="absolute inset-0 h-full w-full backface-hidden rounded-2xl overflow-hidden bg-dark-200 border border-white/10">
          ...
          <div className="relative h-[60%] w-full overflow-hidden">
             ...
             {/* Mode Badge */}
             {mode && (
                 <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md border border-white/20 text-white text-xs font-bold px-3 py-1 rounded-full uppercase flex items-center gap-1">
        ...
        <div className="absolute inset-0 h-full w-full backface-hidden rotate-y-180 rounded-2xl overflow-hidden bg-dark-200 p-6 border-2 border-primary/20 flex flex-col justify-between">
            ...
            <div>
                ...
                <div className="mt-6 space-y-3">
                    {audience && (
                        <div className="flex items-center gap-2 text-sm text-light-200 bg-white/5 p-2 rounded-lg">
                            <Users size={16} className="text-primary" />
                            <span><span className="font-semibold text-white">Target:</span> {audience}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="flex gap-3 items-center pt-4 border-t border-white/10">
                {/* Share Button (Corrected: No onClick wrapper here) */}
                <div className="shrink-0">
                    <ShareEventBtn slug={slug} />
                </div>

                {/* View Button */}
                <div className="flex-1 py-2.5 bg-primary text-black rounded-lg text-center font-bold text-sm hover:bg-white transition-colors">
                    View Details
                </div>
            </div>
        </div>
